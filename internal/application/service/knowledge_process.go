...
func (s *knowledgeService) processChunks(ctx context.Context,
	...
	if options.EnableMultimodel && len(options.StoredImages) > 0 {
		...
		if err == nil {
			...
			// and bounds blast radius if a worker wedges. postProcessOpts()
			// applies that default; tunable via WEKNORA_POSTPROCESS_TIMEOUT.
			// Deterministic TaskID is required for the reconciler to
			// distinguish a live post-process from an orphaned one.
